...
public class DBHelper extends SQLiteOpenHelper {
	...
	public long insertProduct(Product product) {
		...
		try {
			...
			// Sets the created date and the modified date to the current time.
			if (code != -1) {
				getWritableDatabase().execSQL(
						"UPDATE " + TABLE_NAME + " SET " + COLUMN_CREATED
								+ " = datetime('now'), " + COLUMN_MODIFIED
								+ " = datetime('now') WHERE " + COLUMN_ID
								+ " = " + code + ";");
			}
		} catch (Exception e) {
			Log.e(tag,
	...
	public boolean updateProduct(Product product) {
		...
		try {
			int numRows = getWritableDatabase().update(TABLE_NAME, cv,
					COLUMN_ID + " = ?", new String[] { String.valueOf(id) });

			// Sets the modified date to the current time.
			if (numRows == 1) {
				getWritableDatabase().execSQL(
						"UPDATE " + TABLE_NAME + " SET " + COLUMN_MODIFIED
								+ " = datetime('now') WHERE " + COLUMN_ID
								+ " = " + id + ";");
				return true;
			}
			...
			Log.e(tag,
					"Exception caught while updating a product:\n"
							+ e.getClass() + " " + e.getMessage());
		}
